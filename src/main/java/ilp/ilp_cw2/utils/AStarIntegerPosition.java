...
public class AStarIntegerPosition {
    ...
    static final double HEURISTIC_MULTIPLIER = 1.1;
     ...
     */
    public static Pair<ArrayList<LngLat>, Double> AStarPathWithCost(LngLat from, LngLat to, double maxCost, List<Raycasting.Line> regionLines) {
        ...
        boundaryQueue.add(new Node(startPosition, null, 0, Utils.getDistance(from, to) * HEURISTIC_MULTIPLIER));
        boundaryMap.put(startPosition, new Node(startPosition, null, 0, Utils.getDistance(from, to) * HEURISTIC_MULTIPLIER));
        ...
        while (!boundaryQueue.isEmpty()) {
            ...
            for (IntegerPosition nextPosition : nextPositions) {
                ...
                double newFCost = newGCost + Utils.getDistance(nextLngLat, to) * HEURISTIC_MULTIPLIER;
